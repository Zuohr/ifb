...
(function($) {
	...
	MultipleSelect.prototype = {
		...
		init : function() {
			...
			if (this.options.selectAll && !this.options.single) {
				html
						.push('<li>', '<label style="font-size:2px">',
			...
			html.push('<li class="ms-no-results">'
					+ this.options.noMatchesFound + '</li>');
			html.push('</ul>');
			this.$drop.html(html.join(''));

			this.$drop.find('ul').css('max-height',
					this.options.maxHeight + 'px');
			this.$drop.find('.multiple').css('width',
					this.options.multipleWidth + 'px');

			this.$searchInput = this.$drop.find('.ms-search input');
			this.$selectAll = this.$drop.find('input[' + this.selectAllName
					+ ']');
			this.$selectGroups = this.$drop.find('input['
					+ this.selectGroupName + ']');
			this.$selectItems = this.$drop.find('input[' + this.selectItemName
					+ ']:enabled');
			this.$disableItems = this.$drop.find('input[' + this.selectItemName
					+ ']:disabled');
		...
		optionToHtml : function(i, elm, group, groupDisabled) {
			...
			if ($elm.is('option')) {
				var value = $elm.val(), text = $elm.text(), selected = (that.$el
						.attr('multiple') != undefined) ? $elm.prop('selected')
						: ($elm.attr('selected') == 'selected'), style = this.options
						.styler(value) ? ' style="'
						+ this.options.styler(value) + '"' : '';

				disabled = groupDisabled || $elm.prop('disabled');
				if ((this.options.blockSeparator > "")
						&& (this.options.blockSeparator == $elm.val())) {
					html.push('<li' + (multiple ? ' class="multiple"' : '')
							+ style + '>', '<label  class="'
				...
				html.push('<li class="group">', '<label class="optgroup'
						+ (disabled ? ' disabled' : '') + '" data-group="'
						+ _group + '">', '<input type="checkbox" '
						+ this.selectGroupName
						+ (disabled ? ' disabled="disabled"' : '') + ' /> ',
						label, '</label>', '</li>');
		...
		update : function(isInit) {
			...
			if (selects.length === 0) {
				$span.addClass('placeholder').html(this.options.placeholder);
					...
					&& selects.length < this.options.minumimCountSelected) {
				$span.removeClass('placeholder').html(
						this.getSelects('text').join(', '));
			} else if (this.options.allSelected
					&& selects.length === this.$selectItems.length
			...
			if (text.length === 0) {
				...
				this.$selectItems
						.each(function() {
							...
							$parent[$parent.text().toLowerCase().indexOf(text) < 0 ? 'hide'
									: 'show']();
						});
	...
	$.fn.multipleSelect = function() {
		...
		value, allowedMethods = [ 'getSelects', 'setSelects', 'enable',
				'disable', 'checkAll', 'uncheckAll', 'focus', 'blur', 'refresh' ];
